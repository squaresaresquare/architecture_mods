...
public class OakLogBlockEntityRenderer implements BlockEntityRenderer<OakLogBlockEntity, OakLogBlockEntityRenderState> {
    ...
    @Override
    public void extractRenderState(OakLogBlockEntity blockEntity, OakLogBlockEntityRenderState state, float tickProgress, @NotNull Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderer.super.extractRenderState(blockEntity, state, tickProgress, cameraPos, crumblingOverlay);
    }

    @Override
    public void submit(OakLogBlockEntityRenderState state, PoseStack matrices, @NotNull SubmitNodeCollector queue, @NotNull CameraRenderState cameraState) {
        matrices.pushPose();
