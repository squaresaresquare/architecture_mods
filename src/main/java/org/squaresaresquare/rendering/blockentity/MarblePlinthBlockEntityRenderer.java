...
public class MarblePlinthBlockEntityRenderer implements BlockEntityRenderer<MarblePlinthBlockEntity, MarblePlinthBlockEntityRenderState> {
    ...
    @Override
    public void extractRenderState(MarblePlinthBlockEntity blockEntity, MarblePlinthBlockEntityRenderState state, float tickProgress, Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderer.super.extractRenderState(blockEntity, state, tickProgress, cameraPos, crumblingOverlay);
    }

    @Override
    public void submit(MarblePlinthBlockEntityRenderState state, PoseStack matrices, SubmitNodeCollector queue, CameraRenderState cameraState) {
        matrices.pushPose();
